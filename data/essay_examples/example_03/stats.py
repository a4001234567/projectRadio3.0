"""
Compute and save stats for Example 3 matrices.
  - Rates and GF(2) rank
  - 6-cycles: exact via matrix-power formula  tr(B0^3)/6 - sum_v C(dv,3)
  - 8/10-cycles: optimised DFS with SIGALRM timeout

BR and Vandermonde (punctured) are algebraically equivalent codes, so their
cycle counts should be identical.
"""
...
matrices = {
    'vandermonde (punctured)': reader(os.path.join(out, 'vandermonde_punctured.zip')),
    'BR':                      reader(os.path.join(out, 'br.zip')),
...
lines = []
lines.append('=' * 64)
lines.append('Example 3 — BR vs Vandermonde J=5, L=37, P=37 (block=36)')
lines.append('=' * 64)
